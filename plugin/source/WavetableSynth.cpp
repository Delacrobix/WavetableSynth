...
std::vector<float> WavetableSynth::generateSineWaveTable()
{
  ...
  const auto PI = std::atanf(1.f) * 4.f;

  for (auto i = 0; i < WAVETABLE_LENGTH; ++i)
  {
    sineWaveTable[i] = std::sinf(2.f * PI * static_cast<float>(i) / static_cast<float>(WAVETABLE_LENGTH));
  }
...
void WavetableSynth::prepareToPlay(double sampRate)
{
...
void WavetableSynth::processBlock(juce::AudioBuffer<float> &buffer, juce::MidiBuffer &midiMessages)
{
  ...
  {
    const auto midiEvent = midiMessage.getMessage();  // get the midi message
...
float WavetableSynth::midiNoteNumberToFrequency(int midiNoteNumber)
{
  ...
  constexpr auto A4_NODE_NUMBER = 69.f;
  constexpr auto SEMITONES_IN_AN_OCTAVE = 12.f;

  return A4_FREQUENCY * std::powf(2.f, (midiNoteNumber - A4_NODE_NUMBER) / SEMITONES_IN_AN_OCTAVE);
}
